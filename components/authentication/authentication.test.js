...
async function createUserGroup () {
  const adminUser = await createMockUser(1, null, null, "admin", "mockPassword1", "admin")
...
describe("Authentication route tests", () => {
  ...
  describe("ADMIN", () => {
    describe("Create admin account", ()=> {
      test("creates admin account correctly", async ()=> {
        const res = await request.post(
          '/api/auth/admin-create'
        )
        .set({...HEADERS.formUrlEncoded, "x-api-key": process.env.ADMIN_TOKEN })
        .send({
        ...
        const adminUser = await User.findOne({ email: "admin@example.com"}).exec();
  ...
  describe("SIGN UP", ()=> {
    ...
    test("Sign up fails because user already exists", async() => {
    ...
    test("Account is created successfully", async() => {
      const res = await request.post("/api/auth/signup")
        .set(HEADERS.formUrlEncoded)
        .send(mockUser)
    ...
    describe("getUserById", () => {
      describe("ADMIN", ()=> {
        test("200 - get any user by Id", async()=> {
          ...
          // log the admin in
          const loginRequest = await request.post("/api/auth/login")
            .set(HEADERS.formUrlEncoded)
          ...
          const res = await request.get(`/api/auth/user/${normalUsers[0]._id}`)
        ...
        test("404 - get user that is not found", async() => {
          ...
          // return 404
          const res = await request.get(`/api/auth/user/${id.toString()}`)
            .set({ 'authorization': `Bearer ${adminLoginToken}`})
            .send()
        ...
        test("500 - Invalid mongoObjectId gives a server error", async() => {
          ...
          const adminLoginToken = loginRequest.body.tok
          // Return 500
          const res = await request.get(`/api/auth/user/1`)
            .set({ 'authorization': `Bearer ${adminLoginToken}`})
            .send()
      ...
      describe("USER", ()=> {
        test("401 - tries to access another user by id", async() => {
          ...
          const mockUser = await createMockUser(1, undefined, undefined, "bigtest", "mockPassword1")
          const loginRequest = await request.post("/api/auth/login")
            .set(HEADERS.formUrlEncoded)
            .send({ email: mockUser[0].email, password: "mockPassword1" })

          const { tok } = loginRequest.body
          const req = await request.get(`/api/auth/user/${normalUsers[0]._id.toString()}`)
        ...
        test("200 - Access own user id", async() => {
          ...
          const { tok } = loginRequest.body
          const req = await request.get(`/api/auth/user/${mockUser[0]._id.toString()}`)
            .set({ 'authorization': `Bearer ${tok}`})
            .send()
  ...
  describe("PASSWORD RECOVERY", () => {
    describe("POST request password recovery", () => {
      test("200 - user not found - get correct response message", async () => {
        const req = await request.post("/api/auth/password-recovery/request")
          .set(HEADERS.formUrlEncoded)
      ...
      test("Check that methods are called properly", async() => {
        const jwtManagerSpy = jest.spyOn(JWTManager, "sign")
        const generatePasswordRecoveryURLSpy = jest.spyOn(Helpers, "generatePasswordRecoveryURL").mockReturnValueOnce("mockUrl");
        const emailServiceClientSpy = jest.spyOn(emailServiceClient, "sendEmail").mockImplementation(() => Promise.resolve())
    ...
    describe("POST Recovery - UpdatePassword", () => {
      test("500 - malformed token", async () => {
        const req = await request.post("/api/auth/password-recovery/update-password")
          .set(HEADERS.formUrlEncoded)
      ...
      test("400 - Invaid or expired token", async () => {
        const jwtSpy = jest.spyOn(JWTManager, "verify").mockImplementation(() => Promise.resolve({ recoveryToken: "dummyToken "}))
        const req = await request.post("/api/auth/password-recovery/update-password")
          .set(HEADERS.formUrlEncoded)
          .send({ password: "Password$123", token: "fakeToken" });
      ...
      test("201 - recovery flow", async () => {
        ...
        jest.spyOn(emailServiceClient, "sendEmail").mockImplementation(() => Promise.resolve())
        ...
        jest.spyOn(JWTManager, "sign").mockImplementation(() => 
          Promise.resolve(recoveryJWToken)
        )

        // Create the request
        await request.post("/api/auth/password-recovery/request")
          .set(HEADERS.formUrlEncoded)
          .send({ email: user[0].email })
        
        // Attempt the password reset process
        const recoveryRequest = await request.post("/api/auth/password-recovery/update-password")
          .set(HEADERS.formUrlEncoded)
  ...
  describe("EMAIL VERIFICATION", () => {
    test("400 - Email is already verified", async () => {
      ...
      const req = await request.post('/api/auth/verification-code-email')
        .set(HEADERS.formUrlEncoded)
        .send({ email: testUser[0].email, code: "111111" });
      
      expect(req.statusCode).toBe(400);
      expect(JSON.parse(req.error.text).err.includes("E-mail address is already")).toBe(true);
      
    })
    test("400 - Invalid request for the e-mail address", async () => {
      const testUser = await createMockUser(1);
      const verificationDataSpy = jest.spyOn(VerificationData, "createVerificationData").mockImplementation(() => Promise.resolve(null));
    ...
    test("500 verification code is not defined", async () => {
      const testUser = await createMockUser(1);
      const verificationDataSpy = jest.spyOn(VerificationData, "createVerificationData").mockImplementation(() => Promise.resolve({
        _id: "mockId",
        email: testUser[0].email,
        verified: false,
        created: new Date(),
    ...
    test("200 - sends verification code e-mail successfully", async () => {
      const testUser = await createMockUser(1);
      jest.spyOn(VerificationData, "createVerificationData").mockImplementation(() => Promise.resolve({
        _id: "mockId",
        email: testUser[0].email,
        code: "123456",
        verified: false,
      ...
      const emailSenderSpy = jest.spyOn(emailServiceClient, "sendEmail").mockResolvedValue()

      const req = await request.post('/api/auth/verification-code-email')
        .set(HEADERS.formUrlEncoded)
        .send({ email: testUser[0].email })
      
      expect(emailSenderSpy).toHaveBeenCalledWith('/verification-code', { recipient: testUser[0].email, code: "123456" });
      expect(req.statusCode).toBe(200);
    })
    describe("CHECK IF EMAIL IS VERIFIED", () => {
      test("Email is verified expect correct message", async () => {
        const testUser = await createMockUser(1);
        await EmailVerificationModel.create({
          email: testUser[0].email,
          code: "123456",
        ...
        const req = await request.get(`/api/auth/isEmailVerified/${testUser[0].email}`)
          .set(HEADERS.formUrlEncoded)
          .send();
      ...
      test("Email is not verified, expect correct message", async () => {
        const testUser = await createMockUser(1);
        await EmailVerificationModel.create({
          email: testUser[0].email,
          code: "123456",
