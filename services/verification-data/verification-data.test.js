...
import { createVerificationData, isEmailVerified, validateCode } from "./verification-data";
	...
	describe("CreateVerificationData", ()=> {
		...
		describe('Validate code functionality', ()=> {
			...
			test('validation is complete and document is updated correctly', async() => {
				...
				const updatedDoc = await EmailVerificationModel.findById(doc._id).exec();
 ...
 * 
 */
async function createDummyVerificationEntry (email = testEmail, verified=false) {
