...
describe("PLAID tests", () => {
  describe("GET Link token", () => {
    test("200 - Link token created", async () => {
      ...
      const plaidInstanceSpy = jest.spyOn(PlaidMock.PlaidApi.prototype, "linkTokenCreate").mockImplementation(() => Promise.resolve({
        data: {
      ...
      const req = await request.get("/api/plaid/get_token")
    ...
    test("500 error response", async () => {
      const testUser = await createMockUser(1);
      const token = await loginUser(request, testUser[0], "Password$123");
      const plaidInstanceSpy = jest.spyOn(PlaidMock.PlaidApi.prototype, "linkTokenCreate").mockImplementation(() => Promise.reject(new Error("Server Error")))
  ...
  describe("Set Public Token", () => {
    test("Testing the public token", async () => {
      ...
      const itemPublicTokenSpy = jest.spyOn(PlaidMock.PlaidApi.prototype, "itemPublicTokenExchange").mockImplementation(() => 
        Promise.resolve({
          data: {
      ...
      jest.spyOn(PlaidMock.PlaidApi.prototype, "linkTokenCreate").mockImplementation(() => Promise.resolve({
        data: {
          expiration: '2023-05-09T22:42:32Z',
          link_token: 'fakeLinkToken',
          request_id: 'fakeRequestId'
      ...
      const req = await request.post("/api/plaid/set_public_token")
        .set({ ...HEADERS.formUrlEncoded, authorization: `Bearer ${token}` })
    ...
    test("500 error response", async () => {
      ...
      // Mock the itemPublicToken
      jest.spyOn(PlaidMock.PlaidApi.prototype, "itemPublicTokenExchange").mockImplementation(() => 
  ...
  describe("Get financial details by userId", () => {
    test("404 - Admin tries to get financial data for user who does not exist", async () => {
      ...
      const testAdmin = await createMockUser(1, undefined, undefined, "admin", undefined, "admin");
      
      // Log both users in
      const testAdminToken = await loginUser(request, testAdmin[0], "Password$123");

      // Create a dummy ID to search
      const dummyId = mongoose.Types.ObjectId();
      const req = await request.get(`/api/plaid/financial_details/${dummyId}?category=balance`)
        .set({ ...HEADERS.formUrlEncoded, authorization: `Bearer ${testAdminToken}` })
        .send()
    ...
    test("400 - Access token is not found on target user", async () => {
      ...
      const req = await request.get(`/api/plaid/financial_details/${testUser[0]._id.toString()}?category=balance`)
        .set({ ...HEADERS.formUrlEncoded, authorization: `Bearer ${testAdminToken}` })
        .send()
    ...
    test("200 Response", async () => {
      ...
      const financialRequestSpy = jest.spyOn(PlaidMock.PlaidApi.prototype, "liabilitiesGet").mockImplementation(() => Promise.resolve({ data: "mockData" }))
    ...
    test("500 response", async () => {
      ...
      jest.spyOn(PlaidMock.PlaidApi.prototype, "liabilitiesGet").mockImplementation(() => Promise.reject(new Error("Error")))
      // Add a token to the user
      testUser[0].plaidAccessToken = "dummyToken";
      await testUser[0].save();
      const req = await request.get(`/api/plaid/financial_details/${testUser[0]._id.toString()}?category=balance`)
